...
function parseModuleFile(filePath) {
  ...
  const sourceFileMatch = src.match(/source: WEB6 AI Layer WebAPI (\S+)\)/);
...
for (const file of files) {
  ...
  const content = `# ${moduleName} — \`web6.${clientProp}\`

Source controller: [\`${sourceFile}\`](https://github.com/NextGenSoftwareUK/OASIS2/blob/main/WEB6/NextGenSoftware.OASIS.Web6.WebAPI/Controllers/${sourceFile})
...
const { isError, message, result } = await web6.${clientProp}.${exampleOp.jsName}(${exampleArgs});
...
const indexRows = moduleSummaries
  .map((m) => `| [\`web6.${m.clientProp}\`](modules/${m.moduleName}.md) | \`${m.routePrefix}\` | ${m.opCount} |`)
  .join('\n');

const totalOps = moduleSummaries.reduce((n, m) => n + m.opCount, 0);

const indexContent = `# WEB6 AI Layer API — JavaScript SDK Reference
